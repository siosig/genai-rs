...
**Baseline**: google-genai 2.19.0 | **Crate**: `google-genai-rs` (`google_genai`) | **Source of truth**: `specs/001-port-genai-rust/contracts/parity-matrix.md`
...
| Python | Rust | Status | Test(s) |
|---|---|---|---|
| `live.connect` | `live::Live::connect` | ✅ Implemented (async only, no sync variant) | `tests/live.rs::connect_rejects_vertex_only_config_field`<br>`tests/live.rs::connect_uses_query_key_and_sends_setup_first`<br>`tests/live.rs::sending_after_the_server_closes_the_connection_fails`<br>and 5 more |
